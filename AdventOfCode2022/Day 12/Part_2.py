    ...
    for j in range(len(arr[0])):
        for adj in [(i+1,j),(i-1,j),(i,j+1),(i,j-1)]:
            if adj[0] >= 0 and adj[0] < len(arr) and adj[1] >= 0 and adj[1] < len(arr[0]):
                if arr[i][j] <= arr[adj[0]][adj[1]] + 1:
                    g.add_edge(j+(i*len(arr[0])), adj[1]+(adj[0]*len(arr[0])))

D, P = Graph.dijkstra(g, dest)

print(min(*[ D[val] for val in [ val + (row*len(arr[0])) for row in range(len(arr)) for val in range(len(arr[0])) if arr[row][val] == 0 ]]))
